...
def parse_wavelog_instances(raw_instances: Any) -> list[WavelogInstance]:
    ...
    for index, item in enumerate(raw_instances):
        ...
        if not isinstance(url, str) or not url.strip():
            raise RuntimeError(
                f"wavelog_instances[{index}].url must be a non-empty string"
            )

        if not isinstance(api_key, str) or not api_key.strip():
            raise RuntimeError(
                f"wavelog_instances[{index}].api_key must be a non-empty string"
            )
...
class HamlibRigClient:
    ...
    def error_message(self) -> str:
        ...
        return (
            message.strip() if isinstance(message, str) else str(self.last_error_status)
        )
    ...
    def _relative_power_to_watts(
        self, relative_power: float, frequency_hz: float, mode: int
    ) -> float | None:
    ...
    def read_state(self) -> RadioState | None:
        ...
        if relative_power is not None:
            power_watts = self._relative_power_to_watts(
                relative_power, frequency_hz, mode
            )
...
def run(config: AppConfig, *, dry_run: bool = False) -> int:
    ...
    try:
        while True:
            if not rig_client.connect():
                message = (
                    f"rig_open() failed. Retrying in {RETRY_INTERVAL_SECONDS} seconds."
                )
            ...
            if state is None:
                message = f"read_state() failed. Retrying in {RETRY_INTERVAL_SECONDS} seconds."
                if rig_client.last_error_status < 0:
                    message += f" Hamlib: {rig_client.error_message()}"
                eprint(message)
            ...
            if dry_run:
                ...
                for instance in config.wavelog_instances:
                    if not post_info_to_wavelog(session, instance, payload):
                        eprint(
                            f"Failed to post to Wavelog instance at {instance.url}; API key: {instance.api_key}"
                        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Wavelog-Hamlib interface using Python and Hamlib"
    )
    parser.add_argument("--config", help="Path to config.json")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Read rig data and print payload without POSTing to Wavelog",
    )
